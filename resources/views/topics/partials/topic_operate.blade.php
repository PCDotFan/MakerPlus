<div class="panel-footer uk-panel-teaser operate">
    ...
    <p class="uk-text-right uk-margin-bottom-remove actions">
        ...
        @endif @if ($topic->order
        <=0 ) <a data-ajax="post" id="topic-sink-button" href="javascript:void(0);" data-url="{{ route('topics.sink', [$topic->id]) }}" class="admin uk-button {{ $topic->order < 0 ? 'active' : '' }}" data-uk-tooltip title="沉贴，帖子将会被降低排序优先级">
            ...
            @endif @if ( $currentUser && ($manage_topics || $currentUser->id == $topic->user_id) )
