@extends('layouts.default') @section('title') {{{ $topic->title }}}_@parent @stop @section('description') {{{ $topic->excerpt }}} @stop @push('addon')
    ...
    <div class="pk-width-content uk-row-first">
        ...
        <div class="votes-container uk-panel uk-panel-box uk-margin padding-md">
            <div class="panel-body vote-box text-center">
                <div class="btn-group">
                    <a data-ajax="post" href="javascript:void(0);" data-url="{{ route('topics.upvote', $topic->id) }}" title="{{ lang('Up Vote') }}" data-content="点赞相当于收藏，可以在个人页面的「赞过的话题」导航里查看" id="up-vote" class="vote btn btn-primary {{ $topic->user->payment_qrcode ?: 'btn-inverted' }} popover-with-html {{ $currentUser && $topic->votes()->ByWhom(Auth::id())->WithType('upvote')->count() ? 'active' :'' }}">
...
$(document).ready(function() {
    var $config = {
        title: '{{{ $topic->title }}} from PHPHub - PHP，Laravel的中文社区 #laravel# @phphub {{ $topic->user->id != 1 ? '
        @李桂龙_CJ ' : '
        ' }} {{{ $topic->user->weibo_name ? '
        @ '.$topic->user->weibo_name : '
        ' }}}',
